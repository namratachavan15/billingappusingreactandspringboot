...
import { FaCalendar, FaTruck, FaFileInvoice, FaCube, FaChartBar, FaRupeeSign, FaPercentage, FaPlay } from "react-icons/fa";
import { usePurchaseReport } from "../States/PurchaseReportContext";

const SupplierWisePurchaseReport = () => {
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const { supplierRows: rows, loadSupplierWise, loading } = usePurchaseReport();
  ...
  const handleGenerate = async () => {
    ...
    await loadSupplierWise({ fromDate, toDate });
  };
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((r, i) => (
                <tr key={i}>
                  <td>
                    <strong style={{ color: 'var(--primary)' }}>{r.supplierName}</strong>
                  </td>
                  <td>
