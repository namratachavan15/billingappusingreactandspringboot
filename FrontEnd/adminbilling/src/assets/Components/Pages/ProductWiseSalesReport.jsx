...
import { FaCalendar, FaBox, FaCube, FaChartBar, FaRupeeSign, FaPercentage, FaPlay } from "react-icons/fa";
...
const ProductWiseSalesReport = () => {
  ...
  const loadReport = async () => {
    ...
    try {
      const res = await api.get(
        `/api/reports/product-wise-sales?fromDate=${fromDate}&toDate=${toDate}`
      );
      setRows(res.data);
    } catch (error) {
  ...
  const totals = rows.reduce(
    ...
    { qty: 0, taxable: 0, cgst: 0, sgst: 0, gst: 0, net: 0 }
  );
