...
import { FaCalendar, FaFileInvoice, FaCube, FaChartBar, FaRupeeSign, FaPercentage, FaPlay } from "react-icons/fa";
import { usePurchaseReport } from "../States/PurchaseReportContext";

const DailyPurchaseReport = () => {
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const { dailyRows: rows, loadDaily, loading } = usePurchaseReport();
  ...
  const handleGenerate = async () => {
    ...
    await loadDaily({ fromDate, toDate });
  };

  const totals = rows.reduce(
    (a, r) => {
      a.totalPurchases += r.billCount || 0;
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((r, i) => (
                <tr key={i}>
                  <td>
                    <strong>{formatDate(r.purchaseDate)}</strong>
                  </td>
                  <td>
